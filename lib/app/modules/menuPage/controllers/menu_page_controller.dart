...
class MenuPageController extends GetxController with StateMixin {
  ...
  publicAddCartMenu(cartItem, checkItem) async {
    logger.info(
        '-- publicAddCartMenu cartItem: $cartItem, checkItem: $checkItem --');
    ...
    try {
      ...
      ordersqlcontroller.getCardList();
      result = true;
      logger.info('-- publicAddCartMenu success --');
      //更改显示购物车价格
      getCartPriceTotal();
    } catch (e) {
      print(e);
...
//限量商品请求接口
  ...
  _addToCartCallback(item, price, options, optionTitle) async {
    ...
    logger.info(
        '-- _addToCartCallback price = $price, options = $options, optionTitle = $optionTitle --');
