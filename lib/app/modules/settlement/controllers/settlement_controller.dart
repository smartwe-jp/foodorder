...
class SettlementController extends GetxController with StateMixin {
  ...
  _showPosCancelEasyLoading(resultString, {resultPFSString: ""}) {
    ...
    if (resultString == "M10") {
      ...
      _showTag = Align(
        child: Text(
            GString.getToString(
                checkLanguage.value, "settlement_posPay_error_connect_worker"),
            style: TextStyle(fontSize: ScreenAdapter.fontSize(28))),
        alignment: Alignment(0, 0),
      );
      _showTagContent = GString.getToString(
          checkLanguage.value, "settlement_posPay_error_connect_worker");
    } else if (resultString == "L06") {
      //需要从端末点击返回
      _showTag = Align(
        child: Text(
            GString.getToString(
                  ...
                  child: Container(
                    ...
                    child: Image.asset(
                        GImage.getImageString(
                            "imgpublic", "printticketloading"),
                        fit: BoxFit.fitHeight),
                  ),
                ),
                Text(
                    GString.getToString(
                        checkLanguage.value, "settlement_posPay_loadint_title"),
                    style: TextStyle(
            ...
            Container(
              ...
              child: FadeInImage(
                placeholder: AssetImage(
                    'assets/images/public/placeholder.png'), // 本地assets中的占位符图像
                image: AssetImage(GImage.getImageString("imgpublic",
                    "settlement_pos_loading_${checkLanguage.value}")),
  ...
  _showScanCodeNoOpenDialog(checknum, showContent, {payType: "qr"}) {
    ...
    Get.dialog(DialogUtils.alertOneButton(show_dialog_content,
        title: GString.getToString(checkLanguage.value, "tag_title"),
        confirmtitle:
            GString.getToString(checkLanguage.value, "tag_button_yes"),
        confirm: () {
...
//刷卡机nfc支付汇报
    ...
    if (printType == "10") {
      if (wlan_print_ip.value != null && wlan_print_ip.value != "") {
        ...
        if (is_allow_wlanPrint_continuous.value == "1") {
          _wifiNetworkReceiptPrintContinuousData(
              serialNumber, printData, takeOut, orderTime, wlan_print_ip.value);
        } else {
          _wifiNetworkReceiptPrintData(
              serialNumber, printData, takeOut, orderTime, wlan_print_ip.value);
        }
      } else {
        return;
      ...
      if (wlan_print_ip_two.value != null && wlan_print_ip_two.value != "") {
        printerIpInfo = {
          "printer_ip": wlan_print_ip_two.value,
          "printer_port": wlan_print_port_two.value,
        };
        if (is_allow_wlanPrint_Two_continuous.value == "1") {
          _wifiNetworkReceiptPrintContinuousData(serialNumber, printData,
              takeOut, orderTime, wlan_print_ip_two.value);
        } else {
          _wifiNetworkReceiptPrintData(serialNumber, printData, takeOut,
              orderTime, wlan_print_ip_two.value);
        }
      } else {
        return;
      }
    ...
    for (var i = 0; i < printData.length; i++) {
      ...
      PictureGeneratorProvider.instance.addPicGeneratorTask(
        PicGenerateTask<PrinterInfo>(
          tempWidget: wifiNetPrintReceiptnew(
                  serialNumber, printData[i], takeOut, orderTime, printer_ip)
              as ATempWidget,
  ...
  Widget wifiNetPrintReceiptnew(
      serialNumber, orderprintData, takeOut, orderTime, printer_ip) {
    var lineHight = 120;
    int menuNum = 0;
    var optionNum = 0;
    int addRowHight = 0;
  ...
  wifiNetPrintLabelnew(orderprintData) async {
    Future.delayed(Duration(milliseconds: 1200), () async {
      ByteData byteData =
          await WidgetToImage.widgetToImage(menuData(orderprintData));
  ...
  Widget menuData(orderprintData) {
    return LabelConstrainedBox(Padding(
